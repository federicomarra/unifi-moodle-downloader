...
async function scrape(secrets, config) {
  ...
  await page.waitForSelector('button[name="submitBtn"][type="submit"]');
  (await page.$('button[name="submitBtn"][type="submit"]')).evaluate(el => el.click());  // Puppeteer's own `click` API sometimes fails
